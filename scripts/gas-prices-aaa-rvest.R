...
#national prices
page <- read_html("https://gasprices.aaa.com/")

prices <- page %>% 
  html_element(xpath = '//*[@id="maincontent"]/div[3]/div[3]/table') %>% 
  html_table() %>% 
  clean_names() %>% 
  rename(period = x, 
         unleaded = regular,
...
national_prices <- prices %>%
  ...
  mutate(across(c(Regular, `Mid-Grade`, Premium, Diesel), ~ as.numeric(str_replace(., "\\$", "")))) %>% 
...
description <- paste0("On ", prices_updated_date_pretty, ", the average cost of gas nationwide was <b>$", round(today_price, 2), " per gallon</b>. That's <b>$",round(abs(today_vs_yesterday), 2), " ", ifelse(today_vs_yesterday > 0, "higher", "lower"), "</b> than the day before, <b>$",round(abs(today_vs_last_month), 2), " ", ifelse(today_vs_last_month > 0, "higher", "lower"), "</b> than a month ago and <b>$",round(abs(today_vs_last_year), 2), " ", ifelse(today_vs_last_year > 0, "higher", "lower"), "</b> than a year ago.")

line_1y_description <- paste0('On ', prices_updated_date_pretty, ', the average cost of gas nationwide was <b>$', round(today_price, 2), ' per gallon</b>. That is <b>$',round(abs(today_vs_yesterday), 2), ' ', ifelse(today_vs_yesterday > 0, 'higher', 'lower'), '</b> than the day before, <b>$',round(abs(today_vs_last_month), 2), ' ', ifelse(today_vs_last_month > 0, 'higher', 'lower'), '</b> than a month ago and <b>$',round(abs(today_vs_last_year), 2), ' ', ifelse(today_vs_last_year > 0, 'higher', 'lower'), '</b> than a year ago. <br><br>Choose a time frame : <br><span style="line-height:30px"><a target="_self" href="https://datawrapper.dwcdn.net/FinjE/" style="background:#014c12; padding:1px 6px; border-radius:5px; border:1px solid black; color:#ffffff; font-weight:400; cursor:pointer;"> 1 year </a> &nbsp;<a target="_self" href="https://datawrapper.dwcdn.net/VKUkr/" style="background:#858585; padding:1px 4px; border-radius:5px; color:#ffffff; font-weight:400; box-shadow:0px 0px 7px 2px rgba(0,0,0,0.07); cursor:pointer;"> 5 years </a> &nbsp;<a target="_self" href="https://datawrapper.dwcdn.net/FveIx/" style="background:#858585; padding:1px 4px; border-radius:5px; color:#ffffff; font-weight:400; box-shadow:0px 0px 7px 2px rgba(0,0,0,0.07); cursor:pointer;"> 30 days </a>')

line_5y_description <- paste0('On ', prices_updated_date_pretty, ', the average cost of gas nationwide was <b>$', round(today_price, 2), ' per gallon</b>. That is <b>$',round(abs(today_vs_yesterday), 2), ' ', ifelse(today_vs_yesterday > 0, 'higher', 'lower'), '</b> than the day before, <b>$',round(abs(today_vs_last_month), 2), ' ', ifelse(today_vs_last_month > 0, 'higher', 'lower'), '</b> than a month ago and <b>$',round(abs(today_vs_last_year), 2), ' ', ifelse(today_vs_last_year > 0, 'higher', 'lower'), '</b> than a year ago. <br><br>Choose a time frame : <br>
...
get_state_gas <- function(state_code) {
  ...
  message("Fetching: ", state_code)
  
  url <- paste0("https://gasprices.aaa.com/?state=", state_code)
  
  state_name <- unname(state_lookup[state_code])
  ...
  tryCatch({
    ...
    prices <- page %>%
      html_element(xpath = '/html/body/main/div[4]/div[3]/table') %>%
      html_table() %>%
      clean_names() %>%
      rename(
        period = x,
      ...
      mutate(unleaded = as.numeric(str_replace(unleaded, "\\$", "")))
  
  }, error = function(e) {
    message("Failed: ", state_code)
    tibble(
...
all_states_prices <- map_dfr(state_abbs, get_state_gas) %>%
  ...
  rename(
    ...
    `Last week` = `Week Ago Avg.`,
    `Last month` = `Month Ago Avg.`,
    `Last year` = `Year Ago Avg.`
  ) %>%
  select(State, Today, Yesterday, `Last week`, `Last month`, `Last year`)
